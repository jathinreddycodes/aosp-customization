...
import android.os.CustomBatteryManager;
import com.android.internal.CustomBattery.ICustomBatteryService;
...
public final class SystemServiceRegistry {
	...
	registerService(Context.CUSTOM_BATTERY_SERVICE, CustomBatteryManager.class,
		new CachedServiceFetcher<CustomBatteryManager>() {
	  @Override
	  public CustomBatteryManager createService(ContextImpl ctx)
	  {
		IBinder b =  ServiceManager.getService("battery");
		ICustomBatteryService service = ICustomBatteryService.Stub.asInterface(b);
		return new CustomBatteryManager(ctx , service);
	  }});

	// CarFuelManager

	registerService(Context.CARFUEL_SERVICE, CarFuelManager.class,
		new CachedServiceFetcher<CarFuelManager>() {
	  @Override
	  public CarFuelManager createService(ContextImpl ctx)
	  {
		IBinder binder = ServiceManager.getService("CarFuel"); //binder name in SystemServer.java
		ICarFuelData service = ICarFuelData.Stub.asInterface(binder);
		return new CarFuelManager(ctx, service);
	  }});
