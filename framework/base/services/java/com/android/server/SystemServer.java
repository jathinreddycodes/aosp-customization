...
public final class SystemServer implements Dumpable {
    ...
    private void startOtherServices(@NonNull TimingsTraceAndSlog t) {
       ...
       if (isWatch) {
        ...
        t.traceEnd();
	// My first custom service.

	t.traceBegin("StartJathinHelloService");
	try {
   	     Slog.i(TAG, "Starting Jathin Hello Service");
    	     ServiceManager.addService("hello", new com.android.server.jathin.HelloService());
	} catch (Throwable e) {
	...
	//My fuel service
	t.traceBegin("StartMyFuelService");
	try{
	    Slog.i(TAG,  "Starting Fuel service");
	    ServiceManager.addService("Fuel", new  com.android.server.bunty.FuelService());
	} catch (Throwable e) {
	    Slog.e(TAG," Failed to start fuelservice", e);
	}
	t.traceEnd();

	// CustombatteryService
	t.traceBegin("StartCustomBatteryservice");
	try{
	   Slog.i(TAG, "Starting custom battery service");
	   ServiceManager.addService("battery", new com.android.server.CustomBattery.CustomBatteryService());
	} catch (Throwable e) {
	    Slog.e(TAG, "Failed to start battery service", e);
	}
	t.traceEnd();


	// CustomEnginerService
	t.traceBegin("Start custom Engine service");
	try{
		Slog.i(TAG, "Starting custom Enginer Service");
		ServiceManager.addService("EngineService", new com.android.server.jathin.CustomEngineService());
	}catch (Throwable e) {
		Slog.e(TAG, "Failed to start Engine service");
	}
	t.traceEnd();

	t.traceBegin("StartCarFuelService");
	try{
		Slog.i(TAG,"STARTING CAR FUEL SERVICE");
		ServiceManager.addService("CarFuel" , new com.android.server.jathin.CarFuelService());
	}catch(Throwable e) {
		Slog.e(TAG, "Failed to start car fuel service");
	}
	t.traceEnd();
